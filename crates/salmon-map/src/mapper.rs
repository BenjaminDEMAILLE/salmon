...
use crate::align::{align_chain, align_in_window, AlignConfig, AlignTask, Alignment};
use crate::collect::{
...
use crate::score::{
    finalize_mappings_counted, finalize_mappings_counted_into, MapScratch, RawMapping, ScoreConfig,
    ScoredMapping,
};
...
impl PendingPair {
    ...
    pub fn finalize<R: RefProvider>(
        ...
        let mut raw = Vec::new();
        let mut ri = 0usize;
        // Mate-rescue scratch. The inline path threads `MapScratch` through; this
        // path has no scratch to borrow, so it keeps one buffer for the whole
        // fragment rather than one allocation per rescued orphan.
